...
namespace Stok_Takip_Programı
{
    public partial class frmMüşteriListele : Form
    {
        ...
        DataSet daset = new DataSet(); //kayıtları geçici olarak tutuyoruz
        ...
        {
            baglanti.Open();
            SqlDataAdapter adtr = new SqlDataAdapter("select *from müşteri", baglanti); //müşteri tablosundaki kayıtları göster
            adtr.Fill(daset, "müşteri"); //geçici tablo ve veri tabanındaki tablo
            dataGridView1.DataSource = daset.Tables["müşteri"]; //datagride aktardım
            baglanti.Close();
        }
        ...
        { //burada datagride cift tıklayınca kayıtlar textbox a geliyor onu ayarladım
        ...
        private void btnSil_Click(object sender, EventArgs e) //silme işlemini yapıyoruz
        {
            baglanti.Open();
